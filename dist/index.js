...
import { createAuthorizationRequest, exchangeCodeForTokens, refreshTokens, USER_AGENT, BETA_FLAGS, } from "./oauth.js";
...
const toCC = (name) => ccLookup.get(name.toLowerCase()) ?? name;
const fromCC = (name) => {
    ...
    async function proactiveRefresh() {
        ...
        try {
            ...
            if (!auth || auth.type !== "oauth" || !auth.refresh)
                return;
            if (auth.expires > Date.now() + REFRESH_BUFFER)
                return;
